...
def litCricFriend(wordList, text):
    ...
    lowerCaseNoMdash = lowerCaseText.replace('--', ' ')
    ...
    splitList = lowerCaseNoMdash.split()
    ...
    for word in splitList:
        newSplitList.append(word.strip(string.punctuation))
    ...
    for word in wordList:
        if word in newSplitList:
            listCount += newSplitList.count(word)
    ...
    wordCount = len(newSplitList)
        
    ratio = listCount/wordCount
    return ratio
...
'''
The results show that the authors use 'an' and 'a' differently. Also, the word 'the' was used differently among the two authors.
'''
